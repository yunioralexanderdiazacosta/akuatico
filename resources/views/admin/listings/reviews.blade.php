...
@section('page_title',__('Listing Reviews'))
@section('content')
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-end">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title">@lang('Listing Reviews')</h1>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item" aria-current="page">@lang('User Listings')</li>
                            <li class="breadcrumb-item active" aria-current="page">@lang('Reviews for - ' .$listing->title)</li>
                        </ol>
                    </nav>
                </div>
            </div>
                ...
                <div class="card-header card-header-content-md-between">
                    <div class="mb-2 mb-md-0">
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            <input id="datatableSearch" type="search" class="form-control" placeholder="Search Reviews"
                                   aria-label="Search Listing" autocomplete="off">
                        </div>
                    </div>

                    <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                        @if(adminAccessRoute(config('role.listing_reviews.access.delete')))
                            ...
                            <div
                                ...
                                <div class="card">
                                    <div class="card-header card-header-content-between">
                                        <h5 class="card-header-title">@lang('Filter Reviews')</h5>
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-12 mb-4">
                                                <small class="text-cap text-body">@lang("Select User")</small>
                                                <div class="tom-select-custom">
                                                    <select
                                                        ...
                                                        <option value="all"
                                                                data-option-template='<span class="d-flex align-items-center">@lang("All User")</span>'>
                                                            @lang("All User")
                                                        </option>
                                                        @foreach($userReviews as $reviewUser)
                                                            <option value="{{ optional($reviewUser->review_user_info)->id }}"
                                                                    data-option-template='<span class="d-flex align-items-center">
                                                                    {{ optional($reviewUser->review_user_info)->firstname.' '.optional($reviewUser->review_user_info)->lastname }}</span>'>
                                            ...
                                            <div class="col-sm-12 mb-4">
                                                <small class="text-cap text-body">@lang("Select Rating")</small>
                                                <div class="tom-select-custom">
                                                    <select
                                                        class="js-select js-datatable-filter form-select form-select-sm"
                                                        id="select_rating"
                                                        ...
                                                        <option value="all"
                                                                data-option-template='<span class="d-flex align-items-center">@lang("All Rating")</span>'>
                                                            @lang("All Rating")
                                                        </option>
                                                        @for($i = 5; $i > 0; $i--)
                                                            <option value="{{ $i }}"
                                                                    data-option-template='<span class="d-flex align-items-center">{{ $i .' Star' }}</span>'>
                                                                @lang( $i .' Star')
                                                            </option>
                                                        @endfor
                                                    </select>
            ...
            <script>
                $(document).on('ready', function () {
                    ...
                    HSCore.components.HSDatatables.init($('#datatable'), {
                        ...
                        ajax: {
                            url: "{{ route('admin.listing.reviews.search', '') }}" + '/' + listingId,
                    ...
                    $(document).on('click', '#filter_button', function () {
                        let filterSelectedUser = $('#select_user').val();
                        let filterSelectedRating = $('#select_rating').val();
                        let filterDate = $('#filter_date_range').val();
                        const datatable = HSCore.components.HSDatatables.getItem(0);
                        datatable.ajax.url("{{ route('admin.listing.reviews.search','') }}" + '/' + listingId + "?filterUser=" + filterSelectedUser + "&filterRating=" + filterSelectedRating + "&filterDate=" + filterDate).load();
                    });
                    ...
                    $(document).on('click', '.delete-multiple', function (e) {
                        ...
                        $.ajax({
                            ...
                            url: "{{ route('admin.listing.reviews.delete.multiple') }}",
                            data: {strIds: strIds},
                            dataType: 'json',
                            type: "post",
