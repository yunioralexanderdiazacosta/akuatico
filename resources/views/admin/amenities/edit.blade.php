                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">@lang('Amenities')</li>
                        </ol>
                    </nav>
                ...
                <div class="tab-content mt-2" id="myTabContent">
                    @foreach($languages as $key => $language)
                        ...
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lang-tab-{{ $key }}" role="tabpanel">
                            <form method="post" action="{{ route('admin.amenities.update',[$id,$language->id]) }}" class="mt-4" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-sm-12 col-md-4 col-lg-4 mb-3">
                                        <div class="form-group">
                                            <label class="form-label" for="name"> @lang('Amenity Name') </label>
                                            <input type="text" name="name[{{ $language->id }}]"
                                                   class="form-control  @error('name'.'.'.$language->id) is-invalid @enderror"
                                                   value="<?php echo old('name'.$language->id, isset($amenityDetails[$language->id]) ? $amenityDetails[$language->id][0]->title : '') ?>">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    <input type="text" name="icon"
                                                           value="<?php echo old('icon'.$language->id, isset($amenityDetails[$language->id]) ? optional($amenityDetails[$language->id][0]->amenity)->icon : '') ?>"
                                                            ...
                                                            <div class="row align-items-center">
                                                                <div class="col">
                                                                    <span class="d-block fs-6 text-body">
                                                                        @lang('If you want to make Deactive then switch off ')
                                                                    </span>
                                                                </div>
                                                                <div class="col-auto">
                                                                    <label class="row form-check form-switch mb-3" for="status">
                                                                    <span class="col-4 col-sm-3 text-end">
                                                                        <input type='hidden' value='0' name='status' {{ optional($amenityDetails[$language->id][0]->amenity)->status == 0 ? 'checked' : '' }}>
                                                                        <input
                                                                            class="form-check-input @error('status') is-invalid @enderror"
                                                                            type="checkbox"
                                                                            name="status"
                                                                            id="status"
                                                                            value="1" {{ optional($amenityDetails[$language->id][0]->amenity)->status == 1 ? 'checked' : '' }}>
                                                                        </span>
                                                                        @error('status')
