...
class ListingCategoryController extends Controller
{
    ...
    public function listingCategorySearch(Request $request)
    {
        ...
        $listingCategories = ListingCategory::query()
            ->with(["details", "parent.details"])
            ->orderBy("id", "desc")
            ->when(!empty($search), function ($query) use ($search) {
                return $query->whereHas("details", function ($q2) use (
                    $search,
                ) {
            ...
            ->when(!empty($request->filterDate) && $endDate == null, function (
                $query,
            ) use ($startDate) {
        ...
        return DataTables::of($listingCategories)
            ->addColumn("checkbox", function ($item) {
                return ' <input type="checkbox" id="chk-' .
                    $item->id .
                    '"
                           class="form-check-input row-tic tic-check" name="check" value="' .
                    $item->id .
                    '"
                           data-id="' .
                    $item->id .
            ...
            ->addColumn("category", function ($item) {
                ...
                if ($item->parent_id) {
                    $parentName = optional($item->parent->details)->name;
                    $name = "<small class='text-muted'>$parentName ></small> " . $name;
                }
                return '<div class="d-flex align-items-center">
                            <i class="' .
                    $item->icon .
                    '"></i>
                            <span class="d-block mb-0 ps-3">' .
                    $name .
                    '</span>
                        </div>';
            })
            ->addColumn("status", function ($item) {
                ...
                return '<span class="badge bg-soft-' .
                    $badgeClass .
                    '"><span class="legend-indicator bg-' .
                    $legendBgClass .
                    '"></span>' .
                    $status .
                    "</span>";
            ...
            ->addColumn("action", function ($item) {
                ...
                $canEdit = adminAccessRoute(
                    config("role.listing_category.access.edit"),
                );
                $canDelete = adminAccessRoute(
                    config("role.listing_category.access.delete"),
                );
                    ...
                    $actions .=
                        '<div class="btn-group">
                            ...
                            <div class="dropdown-menu dropdown-menu-end mt-1" aria-labelledby="productsEditDropdown1">
                                <a class="dropdown-item deleteBtn" href="javascript:void(0)" data-route="' .
                        $deleteUrl .
                        '" data-category-name="' .
    ...
    public function listingCategoryCreate()
    {
        ...
        $allCategories = ListingCategory::onlyParent()->with('details')->get();
        return view("admin.listing_category.create", compact("languages", "allCategories"));
    }
    ...
    public function listingCategoryStore(Request $request, $language)
    {
        ...
        try {
            ...
            if ($request->hasFile("mobile_app_image")) {
                try {
                    $image = $this->fileUpload(
                        $request->mobile_app_image,
                        config("filelocation.listing_category.path"),
                        null,
                        null,
                        "webp",
                        99,
                    ...
                    return back()->with(
                        "error",
                        __("Image could not be uploaded."),
                    );
                }
            }
            ...
            $listingCategory->parent_id = $request->parent_id;
            ...
            return back()->with(
                "success",
                __("Listing Category saved Successfully"),
            );
    ...
    public function listingCategoryEdit($id)
    {
        ...
        $data["allCategories"] = ListingCategory::onlyParent()->with('details')->where('id', '!=', $id)->get();
        $data["listingCategory"] = ListingCategory::findOrFail($id);
        $data["listingCategoryDetails"] = ListingCategoryDetails::with(
            "category",
        )
            ->where("listing_category_id", $id)
            ->get()
            ->groupBy("language_id");
    ...
    public function listingCategoryUpdate(Request $request, $id, $language_id)
    {
        $rules = [
            "name.*" =>
                "required|max:100|unique:listing_category_details,name," .
                $id .
                ",listing_category_id",
        ...
        try {
            ...
            $language = Language::select("id", "default_status")->findOrFail(
                $language_id,
            );
                ...
                if ($request->hasFile("mobile_app_image")) {
                    try {
                        $image = $this->fileUpload(
                            ...
                            "webp",
                            99,
                            $listingCategory->mobile_app_image,
                            $listingCategory->image_driver,
                        );
                        if ($image) {
                            $listingCategory->mobile_app_image = $image["path"];
                            $listingCategory->image_driver = $image["driver"];
            ...
            return back()->with(
                "success",
                __("Listing Category Updated Successfully"),
            );
        } catch (\Exception $exception) {
            DB::rollBack();
            return back()->with("error", __("Something went wrong"));
    ...
    public function listingCategoryDelete($id)
    {
        ...
        try {
            ...
            return back()->with(
                "success",
                __("Listing category has been deleted"),
            );
        } catch (\Exception $exception) {
            DB::rollBack();
            return back()->with("error", __("Something went wrong"));
    ...
    public function listingCategoryDeleteMultiple(Request $request)
    {
        if ($request->strIds == null) {
            ...
            foreach ($request->strIds as $id) {
                $listingCategory = ListingCategory::with("details")->findOrFail(
                    $id,
                );
                $listingCategory->details()->delete();
                $this->fileDelete(
