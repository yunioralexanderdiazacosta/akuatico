...
class CategoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $categories = ListingCategory::select(
            ...
            ->with(["details:id,listing_category_id,language_id,name"])
            ->where("status", 1)
            ->latest()
            ->get();
        ...
        return view(
            ...
            compact("pageSeo", "categories", "categorySingle"),
        );
    }
    ...
    public function categorySearch(Request $request)
    {
        ...
        if ($character != null) {
            $data["categories"] = ListingCategory::with("details")
                ->whereHas("details", function ($q) use ($character) {
                    $q->where("name", "LIKE", $character . "%");
                })
                ->withCount("get_listings")
                ->where("status", 1)
                ->latest()
                ->get();
        } else {
        ...
        $view = view(
            $this->theme . "frontend.category.partials.renderCategory",
            $data,
        )->render();
